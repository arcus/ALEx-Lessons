...
**ANNOVAR** can accept a `.vcf` file as input. The input parameters can be somewhat complex, but are managable once you know what each does. Here is an example of annotating `filename.vcf` which was aligned against `hg19` with information from RefGene, gnomAD, ClinVar, prediction programs and splicing prediction. 
