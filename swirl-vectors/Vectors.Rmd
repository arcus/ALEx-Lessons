...
Hint: As always, if this window hangs, simply use your browser's reload function and check your code for errors. 
...
Character vectors are also very common in R. Double quotes (NOT smart double quotes) are used to distinguish character objects, as in the following example.
...
To add (or ‘concatenate’) your name to the end of `my_char`, use the `c()` function like this: `c(my_char, "you")`. Try it now, storing the result in a new variable called `my_greeting`. Then take a look at the contents of `my_greeting`.
