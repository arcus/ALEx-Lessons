...
question("From the output, how many _characteristics_ of diamonds do you thing this data contains?",
         ...
         incorrect = "It's the same as the number of variables, which is the same as the number of columns. The output says there are 53940 observations of 10 variables. This is followed by a 10-long list of characteristics (carat, cut, color, etc.) that can apply to diamonds."
)
```
...
question("From the output of `str`, how many _diamonds_ are characterized in this dataset?",
         ...
         incorrect = "The output says there are 53940 observations of 10 variables. This is followed by a 10-long list of characteristics (carat, cut, color, etc.) that can apply to diamonds."
)
```
...
Let's see if `cut` is responsible for this increase by adding another argument to `qplot`: `color = cut`. And since the number of arguments is enough to extend the function call close to the right edge, let's use some line breaks to make the code more readable. 
...
The `car2` field is, in fact, `NA` for these entries, but the `carat` field shows they each had a carat size of .2. What's going on here?
