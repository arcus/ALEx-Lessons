...
You'll need the following packages to perform all the functions in this lesson:
...
First convert the implicit variable `location` into a factor variable. 

The `dplyr` package (which is part of the `tidyverse`) was designed with just this sort of data manipulation in mind. 

First, put `dplyr` in the library. Then use it to convert the various locations to a `factor`. This is the sort of function you'll want to do many times in R, so keep this code handy. It creates a new data set, `aam_data`, by mutating `mussels` to have a new column, a factor variable called `location` with levels drawn from 
...
The `lm` function in the native `stats` package fits a linear model by [least squares](https://rcompanion.org/handbook/G_05.html), and can be used for a variety of analyses: [regression](../sb-correlation-and-linear-regression/), analysis of variance, and [analysis of covariance](https://rcompanion.org/rcompanion/e_04.html), for instance.  The analysis of variance is then conducted either with the `anova` function in the `car` package for [Type II or Type III sum of squares](https://rcompanion.org/rcompanion/d_04.html), or with the `anova` function in the native `stats` package for [Type I sum of squares](https://rcompanion.org/rcompanion/d_04.html). 
