...
All statistical tests rest upon assumptions about your data. In order to perform proper statistical analysis, you need to know what assumptions you are making and you need to be certain that none of your assumptions violate the assumptions of the oneway ANOVA. 
...
A oneway ANOVA assumes that your data are homoscedastic, meaning the standard
deviations are equal in the groups. You should examine the standard deviations in the
different groups and see if there are big differences among them. Try that now with `mussels`. Use `sapply` to check the sd for each column of `mussels`. Remember that `sd` needs to know what to do with `NA`s. Wrap the whole function call in a call to `range`. 
...
each group, then a oneway ANOVA is not very sensitive to heteroscedasticity (different
...
question("Which _p_ value is recommended by Jim Berger in his article in The American Statistician *Beyond _p_ < .05* volume?",
         answer("p < .10"),
         answer("p < .05"),
...
*Interpretation*: Since there are `r power_results$groups` groups, you’d need `r power_results$n` observations per group to have an 80% chance of finding a significant result at $\displaystyle \alpha = $) `r power_results$sig.level` if an effect is actually there.
