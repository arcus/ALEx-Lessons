...
One-way ANOVA assumes that **the observations within each group are normally distributed**. This assumption applies to raw data rather than to residuals. 

The ANOVA is _not_ particularly sensitive to deviations from this assumption; if you apply
one-way ANOVA to data that are non-normal, your chance of getting a _p_ value less than
...
histogram, either using the [base plotting system](../sb-base-plotting-system/) or [`ggplot2`](../swirl-ggplot2-part-1/). If the residuals look severely non-normal, try various data transformations and see if one makes the data look more normal. 
...
question("Which of these can be used to transform scalar data points?",
         ...
         random_answer_order = TRUE,
         allow_retry = TRUE,
         correct = "That's right. Basically anything you can do to a number is a transformation of it. Some transformations are more useful than others, but you can try anything.",
         incorrect = "Keep trying. Does a square root transform a number? Does adding 1 to it transform it?"
)
```

Just for fun, try transformating `long_mussels$aam` in various ways in this little sandbox. Send your transformations, as well as the non-transformed data, to `histogram` and find out whether any of your transformations normalize your data. If you're feeling fancy, send the transformed data to `qplot` with the "density" `geom`. HINT: the `ln` function (for taking the natural log) is in the package `radiant.data`.
...
hist(long_mussels$aam)
hist(ln(long_mussels$aam))
qplot(data = long_mussels,
      x = aam^2,
      geom = "density")
```
