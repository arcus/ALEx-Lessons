...
same for the different categories in the categorical variable.     
...
For example, here is some data on a shell measurement (the length of the anterior adductor muscle scar, standardized by dividing by length, **AAM Length**) in the mussel *Mytilus trossulus* from five locations: Tillamook, Oregon; Newport, Oregon; Petersburg, Alaska; Magadan, Russia; and Tvarminne, Finland.

```{r data1, echo=FALSE}
mussels
```

The nominal variable is invisible but implicit in the data structure. It's `location`, with the five values `Tillamook`, `Newport`, `Petersburg`, `Magadan`, and `Tvarminne`. There are six to ten observations of the measurement variable, AAM `length`, from each location. Again, `length` is implicit: you won't find a column called "length", but all the values in the columns we do have are values for `length`. 

The data needs a bit of munging to be ready for analysis. 

```{r select-variables, echo=FALSE}
question("Which variables do we have in the `mussels` data as it is?",
...
First, check to be sure the data is in the correct format for running the analysis. Remember, you should have one categorical variable with 2 or more categories and one measurement (a.k.a. "scalar") variable. 
...
We will need to convert the implicit variable `location` into a factor variable. 

#### Convert Wide Data to Long Data by using `pivot_longer`

The `tidyr` package (which is part of the `tidyverse`) was designed with just this sort of data manipulation in mind. Check out our lesson [Pivot Longer](../sb-pivot-longer/) if you haven't transformed a wide data set to a narrower, longer one. Your work will still be here when you get back. 
...
```{r pivot-in-prep-for-anova, exercise = TRUE, exercise.lines = 5}
...
Quickly visualize `long_mussels` with `boxplot`. This can quickly be done using the R formula interface (`aam_length ~ location`).
...
```
```{r boxplot-hint-3}
boxplot(aam_length ~ location, data = ...)
```
```{r boxplot-solution}
boxplot(aam_length ~ location, data = long_mussels)
```
...
question("Which two packages can you use to run an analysis of variance?",
...
If the analysis of variance indicates an interesting effect of the independent variable (here, `location`), multiple comparisons among the levels of this factor can be conducted using Tukey or Least Significant Difference (LSD) procedures. (The problem of inflating the Type I Error Rate when making multiple comparisons is discussed in the [Multiple Comparisons]() chapter in the Handbook.)  

R functions which make multiple comparisons usually allow for adjusting p-values.  In R, the “BH”, or “fdr”, procedure is the Benjamini–Hochberg procedure discussed in the Handbook.  See ?p.adjust for more information.
