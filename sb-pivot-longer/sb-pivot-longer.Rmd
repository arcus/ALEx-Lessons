...
question("Which variables are we actually interested in from the `mussels` data set?",
         ...
         answer("status of the musself when it was measured (dead or alive)"),
         random_answer_order = TRUE,
         allow_retry = TRUE
)
```

In the above data, each column is a _level_ of an implicit variable, `location`. 

The values in the cells are the standardized lengths of individual mussels' anterior adductor muscle scars (`aam_length`).

Let's make this "wide" data "narrow", using `tidyr`'s function `pivot_longer`.

We want to create a new, narrow data set with two variables: `location`, which has the levels `Tillamook`, `Newport`, `Petersburg`, `Magadan`, and `Tvarminne`, and `aam_length`, which contains a list of standardized anterior adductor muscle lengths, sorted and matched by the various `location` levels. This looks and sounds like it will be complicated, but it isn't.
