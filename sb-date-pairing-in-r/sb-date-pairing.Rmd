...
new_math <- mathr %>% mutate(data = "Math")
new_lang <- langr %>% mutate(data = "Language")
...
question("Where are the `score` modes (as shown by the histograms)?",
         ...
         answer("Around 60"),
         answer("Around 0"),
         correct = "Correct! That's a first-class answer.",
...
If we were to merge these data frames by `id` without limiting by dates, we would have geometric growth. For example, consider subject 697. This subject has 3 took both the math and the language tests three times. A traditional `merge` function will match each _possible_ combination, for 3 X 3 or 9 rows in the resulting data set. Let's try that. Pass `mathr` and `langr` to `merge` and set `all` equal to `TRUE`. Send the results to a new data fram, `merged_df`.
...
Oh dear. Which score is which? Let's make sure we can find out after the merge by creating one new column per data file: `data = "Math"` for the `mathr` matrix and `data = "Language"` for the `langr` matrix. You can do this by piping each matrix to `mutate` to create the new constant. Save the mutate results in `math` and `lang` respectively.
...
mathr %>% mutate(data = "Math")
langr %>% mutate(data = ...)
```
```{r whichscoreiswhich-hint-4}
mathr %>% mutate(data = "Math")
langr %>% mutate(data = "Language")
```
...
Now we can merge again and look at the top of the file.
...
sum(duplicated(merged_df$id))
sum(duplicated(merged_df$date))
```

**EXTRA CREDIT** Write some code to check that the sums of all the `date`s and `id`s add up to the total length of `merged_df`. Your output should be a logical: `TRUE` or `FALSE`.
...
Having so much repetition can causes problems with analyses that assume one observation or one combination of observations per subject. So how can we query the data but limit the number of rows returned to just one row per subject?
...
* Our instruments should be administered only once, so we will probably want to disregard administrations 2-n, no matter when they occurred.     
