...
shh(closest_pairs <- df %>%
      group_by(id) %>%
      filter(abs(date_lang - date_math) == min(abs(date_lang - date_math))) %>%
      slice(1) %>%
      ungroup)
shh(first_math_maybe_language <- earliest_math %>%
      group_by(id) %>%
      filter(abs(date_lang - date_math) == min(abs(date_lang - date_math)) | 
...
If you get an error, don't worry. It probably has something to do with permissions somewhere. You can proceed anyway because we downloaded the data sets for you. If you really, really want to download them for you, try it from your copy of RStudio.
...
question("What should we do if `id` had included numbers outside of the range 1 through 1000?",
...
There are different ways to accomplish this. Here are two queries we might make:
...
1. “I don’t care if it’s a first test or a subsequent readministration, just give me the smallest time interval between a math and a language score for each subject that has both.”   

There are other aspects of this data we need to keep in mind: 
...
Begin by making sure that dates are dates. Change the class of `date` to "date", then check it.
...
question("Which elements are part of the preceding question?",
     ...
     correct = "Well done. That's a run-on sentence pared down to a list of things to do. All we have to do is get the order right and use relevant functions with correct syntax.",
...
Let’s consider what we would do if we just wanted to get the _closest_ (in time) date-matched set of math and language scores---maybe we are doing research into the correlation of scores between the two tests. If study participants have only one of the two tests, math or language, we want to leave them out of the set.

We have already merged the two data sets.

Check out the tibble we created earlier by typing its name (`df`) and running the code (`cmd-Enter` for a Mac, `ctrl-Enter` for a PC).


```{r headdf, exercise = TRUE, exercise.lines = 5}
...
We want to figure out how many tests each person took. Use [piping and summarizing](../sb-piping-and-summarizing/) to figure that out. Start with `df`, pipe it to `group_by` the `id` field, then to `summarise`, setting the new column `size` equal to `n()`, pipe _that_ to `arrange(desc(size))` to list most tests taken to fewest, then pipe those results to `ungroup`.

```{r groupforsize, exercise = TRUE, exercise.lines = 5}
...
Since we're going to be measuring distances between dates rather than using them as factors, we need to make sure they are in `date` format. Make sure they are, using the syntax `date_var <- as.Date(date_var)`. (Substitute the actual variable names for `date_var`.)

```{r datevar, exercise = TRUE, exercise.lines = 5}
...
One last remark before you try this: You'll see a lot of warnings. You can suppress them if you wrap everything in `suppressWarnings`. 

```{r closestpairs, exercise = TRUE, exercise.lines = 5}
...
suppressWarnings(closest_pairs <- df %>%
                   group_by(id) %>%
                   filter(abs(date_lang - date_math) == min(abs(date_lang - date_math))) %>%
                   slice(1) %>%
                   ungroup)
...
## OPTION 2: Earliest Math Test and Nearest Language Test (if any)
