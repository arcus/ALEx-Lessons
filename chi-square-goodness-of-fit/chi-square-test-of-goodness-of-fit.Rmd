...
This lesson is both a reference and a tutorial. It contains all the code samples you need to perform a chi square test of goodness-of-fit. Use the headings to the left to find what you need. You can also learn to understand and perform a chi square test of goodness-of-fit by working through the lesson from start to finish. 

## Variables

>One [nominal variable](http://www.biostathandbook.com/variabletypes.html#nominal)

```{r whatisanominalvariable, echo=FALSE}
question("What is a nominal variable?",
         answer("A variable that classifies observations into discrete categories; also known as a _categorical_ variable", correct = TRUE),
         answer("The relationship between two measurement variables", message = "That's a _ratio_ variable."),
         answer("A variable for which individual observations can be put in order from smallest to largest, even though the exact values are unknown", message = "That's a _ranked_ or _ordinal_ variable."),
         answer("A variable that represents measurements of things", message = "That's a _measurement_ or _scalar_ variable."),
...
question("Differences between category proportions _are present_ for which?",
         answer("The alternative hypothesis", correct = TRUE),
         answer("The null hypothesis", message = "The other one"),
         answer("A shopping list", message = "A shopping list is not particularly relevant here."),
...
The null hypothesis is usually an _extrinsic_ hypothesis, i.e., you knew the expected proportions before doing the experiment. Examples include a 1:1 heads to tails ratio or a 1:2:1 ratio in a genetic cross. Another example would be looking at an area of shore that had 59% of the area covered in sand, 28% mud, and 13% rocks; if you were investigating where seagulls like to stand, your null hypothesis would be that 59% of the seagulls were standing on sand, 28% on mud, and 13% on rocks.

Suggestive results can be reported as “The nominal variable's category proportions were statistically different from theoretical proportions.”
...
```{r defineextrinsic, echo=FALSE}
question("What is an extrinsic hypothesis",
         answer("One based on previously known expected proportions", correct = TRUE),
         answer("One based on proportions estimated by collecting the data", message = "Oops, no."),
         answer("One you can state explicitly", message = "That would be silly."),
         answer("One you keep to yourself"),
     random_answer_order = TRUE,
     allow_retry = TRUE
)
```
...
question("When would you NOT use a chi-square goodness-of-fit test?",
      ...
      correct = "Yes! Have a look at the other answers. You _can_ use a chi square test even when you can't prove your data is normally distributed.",
      random_answer_order = TRUE,
      allow_retry = TRUE
)
```
...
The following code chunk checks to see if you have the required packages and, if you don't, installs them to your R environment. You can copy and paste this code into your local copy of RStudio and run it if you want to be able to duplicate what we do here. 

```{r packages, exercise=FALSE}
...
* We expect a proportion of 3:1 of smooth-winged to wrinkle-winged Drosophila (bonus: say "wrinkle-winged" ten times fast)
...
observed <- c(0.5,0.5)        # REPLACE WITH CORRECT VALUES
expected <- c(0.5,0.5)        # REPLACE WITH CORRECT VALUES
...
Calculate the expected frequency of right-billed birds by multiplying the total sample size (3647) by the expected proportion (0.5) to yield 1823.5. Do the same for left-billed birds. The number of degrees of freedom for an extrinsic hypothesis is the number of classes minus one. In this case, there are two classes (right and left), so there is one degree of freedom.
...
#'
#' You can do this many ways. Here is one somewhat wordy example that arrives at  
#' the correct output. Sometimes you might want to make your code more wordy
#' if you have few concerns about memory or about computation time and if 
#' it is very important that the code be readable. Err on the side of 
#' readability when you can. 
...
Provided $\displaystyle \alpha = .05$, you may reject the null hypothesis that there is no difference between observed and expected proportions because $\displaystyle p = .018$. There are intriguingly more left-billed crossbills than right-billed.

>NOTE: See [Reassessing P Values](../reassessing-p-values/) to find out why I didn't use the word "significant" in the paragraph above.
...
We can't use `chisq.test` because of its faulty assumption about the degrees of freedom. Let's calculate a variable `chi2` manually by plugging the correct values into the formula for a chi square test:
...
Having set up those three values, you can now calculate `chi2` to be the sum of the quantity `observed - expected_count` (which is what `∑(O - E)` means), square it, then that quantity divided by the expected count. To get you started, this is the sum of the differences between `observed` and `expected` squared:
...
The result is not terribly interesting. You cannot reject the null hypothesis that the data fit the expected Hardy-Weinberg proportions. 

>Memorize this forever: *You haven't proven the null*. You can never prove a null hypothesis by deciding you can't with confidence reject it. The bar is very low for proving you can't reject a null hypothesis. It must be much higher before you can with confidence utterly and forever reject the alternative hypothesis and claim the null hypothesis is true.
...
First, set up the input data. This is very similar to what we have done before, but we will also use raw data that I have  loaded for you in a matrix called `trees`. Our observed and expected values are the same as before:
...
Now we plot it. For more information on `ggplot2` see our lessons [ggplot2 Part 1](../swirl-ggplot2-part-1/), [ggplot2 Part 2](../swirl-ggplot2-part-2/), and [ggplot2 Extras](../swirl-ggplot2-extras/). 
...
To do a power analysis using the G*Power program (free software you can download), choose "Goodness-of-fit tests: Contingency tables" from the Statistical Test menu, then choose "Chi-squared tests" from the Test Family menu. To calculate effect size, click on the Determine button and enter the null hypothesis proportions in the first column and the proportions you hope to see in the second column. Then click on the Calculate and Transfer to Main Window button. Set your alpha and power, and be sure to set the degrees of freedom (Df); for an extrinsic null hypothesis, that will be the number of rows minus one.
...
## But I _like_ math. Where's the math?
