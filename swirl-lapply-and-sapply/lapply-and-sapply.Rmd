...
Each of the *`apply` functions will __SPLIT__ up some data into smaller pieces, __APPLY__ a function to each piece, then __COMBINE__ the results. [Hadley Wickham's _Journal of Statistical Software_ paper, "The Split-Apply-Combine Strategy for Data Analysis"](https://www.jstatsoft.org/article/view/v040i01), contains a more detailed discussion of this strategy.
...
This tells us that there are `r N` flags, or observations, and 30 columns, or variables. Each observation is a country and each variable describes some characteristic of that country or its flag. 
...
The 'l' in `lapply` stands for 'list'. Type `class(cls_list)` to confirm that `lapply` returned a list. Then send `cls_list` to the function `length` to find out how long is is.
...
question("What does `lapply` ALWAYS return?",
         ...
         answer("the same structure you passed to it", message = "Plausible, but incorrect."),
...
# OR #
lapply(unique_vals, function(elem) {elem[2]})
# or to make it look more like a normal function, albeit with some weird parentheses:
lapply(unique_vals, 
       function(elem) {
         elem[2]
       })
...
The only difference between previous examples and this one is that we are defining and using our own function right in the call to `lapply`. Our function has no name and disappears as soon as `lapply` is done using it (I like to think of them as 'ephemeral functions' for that reason). These so-called 'anonymous functions' can be very useful when one of R's built-in functions isn't an option.
