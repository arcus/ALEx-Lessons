...
A "grammar" of graphics means that `ggplot2` contains building blocks with which you can create your own graphical objects. What are these basic components of `ggplot2` plots, you ask? There are 7 of them. 
...
Now we'll see how `ggplot` works. We'll build up a similar plot using the basic components of the package. We'll do this in a series of steps. 
...
Notice that you didn't have to pass any arguments to the function `geom_point`. That's because the object `g` has all the data stored in it. (Remember you saw that when you ran `summary` on `g` before.) 

## `ggplot` Scatterplot with Smoothed Regression Line

Now use the expression you just typed (`g + geom_point()`) and add to it another layer, a call to `geom_smooth()`. Notice the messge R gives you.
...
question("What method did `geom_smooth` use to calculate the regression line?",
         ...
         incorrect = "Read the red output from the last plot you made."
)
```
...
Changing the smoothing function to "lm" (linear model) `ggplot2` generated a regression line through the data. 

## `ggplot` Scatterplots in Facets, with Regression Lines

Now copy the expression you just used and add to it another call, this time to the function `facet_grid`. Use the formula `. ~ drv` as it argument. Note that this is the same type of formula used in the calls to `qplot`. 
...
Notice that this is similar to the plot you created at the start of the lesson using `qplot`. (We used a different smoothing function than previously.)
...
Now copy the expression you just typed and add a call to the function `ggtitle` with the string argument "Swirl Rules!".
...
question("Which of the following are customization options you can use with `ggplot`?",
         ...
         random_answer_order = TRUE,
         allow_retry = TRUE,
         correct = "You are really on a roll!",
...
Notice that all these aesthetic arguments are set equal to constants _inside_ calls to `geom`s. We are not setting the aesthetics for the entire plot, but rather for the objects under the control of the `geom` call. 
...
Notice the different shades of pink? That's the result of the `alpha` aesthetic, which you set to 1/2. This aesthetic tells `ggplot` how transparent the points should be. 
...
Use the base plotting function `plot` to create a line plot of this data. Call it with 4 arguments: `myx`, `myy`, `type = "l"`, and `ylim = c(-3, 3)`. The `type = "l"` tells `plot` you want to display the data as a line instead of as a scatterplot.
...
Notice how `plot` plotted the points in the (-3, 3) range for y-values. The outlier at (50, 100) is NOT shown on the line plot. 

Now we'll plot the same data with `ggplot.` Recall that the name of the dataframe is `testdat.` Create the graphical object `o` with a call to `ggplot` with 2 arguments: `testdat` (the data) and a call to `aes` with 2 arguments, `x` set equal to `myx`, and `y` set equal to `myy.`
...
Add a call to `geom_line` with 0 arguments to `g.`
...
Use any of these methods depending on your needs. But first, let's make the plot square by starting with `o`, adding `geom_line()`, adding `ylim = c(-3, 3)`, and then adding one more function with 2 arguments. See if you can guess what that final function should be. 
...
You probably noticed that `ggplot` ordered the data along the x-axis. We didn't ask it to do that; it's just nice that way.
...
Start by creating the graphical object `o` by assigning to it a call to `ggplot` with 2 arguments. The first is the dataset and the second is a call to the function `aes.` This call will have 3 arguments, `x` set equal to `displ`, `y` set equal to `hwy`, and `color` set equal to `factor(year)`. This last will allow us to distinguish between the two manufacturing years (1999 and 2008) in our data.
...
o <- ggplot(mpg, aes(x = displ, ...))
```
```{r complex-plot-hint-4}
o <- ggplot(mpg, aes(x = displ, y = hwy, ...))
```
```{r complex-plot-solution}
o <- ggplot(mpg, aes(x = displ, y = hwy, color = factor(year)))
```

Uh oh! Nothing happened. Does `o` exist? Of course, it just isn't visible yet since you didn't add a visual layer. All you did was assign the data to the plot

```{r if-you-typed-g, echo=FALSE}
question("If you typed `o` at the command line, what would happen?",
         ...
         answer("I would have to this to answer the question", 
...
We'll build the plot up step by step. First, add to `o` a call to the function `geom_point` with 0 arguments.
...
```
```{r familiar-hint-3}
o + geom_point() + facet_grid(...)
```
```{r familiar-hint-4}
o + geom_point() + facet_grid(drv ~ cyl, ...)
```
```{r familiar-solution}
o + geom_point() + facet_grid(drv ~ cyl, margins = TRUE)
```
...
```{r add-or-retype, exercise = TRUE, exercise.lines = 5}
