...
Now we'll plot the same data with `ggplot.` Recall that the name of the dataframe is `testdat.` Create the graphical object `g` with a call to `ggplot` with 2 arguments: `testdat` (the data) and a call to `aes` with 2 arguments, `x` set equal to `myx`, and `y` set equal to `myy.`
...
Instead of adding `ylim(-3, 3)` to the expression `g + geom_line()`, add a call to the function `coord_cartesian` with the argument `ylim` set equal to `c(-3, 3)`.
...
Use any of these methods depending on your needs. But first, let's make the plot square by starting with `g`, adding `geom_line()`, adding `ylim = c(-3, 3)`, and then adding one more function with 2 arguments. See if you can guess what that final function should be. 
...
Uh oh! Nothing happened. Does `g` exist? Of course, it just isn't visible yet since you didn't add a visual layer. All you did was assign the data to the plot

```{r if-you-typed-g, echo=FALSE}
question("If you typed `g` at the command line, what would happen?",
