...
question("Which of these are `dplyr` verbs? Select six.",
         ...
         correct = "You are amazing!",
         incorrect = "It might help if you review [our `dplyr` lesson](../dplyr/).",
         random_answer_order = TRUE,
         allow_retry = TRUE
)
```
...
question("What does a `dplyr` pipe look like?",
         ...
         incorrect = "No, that's not a pipe in `dplyr` terms. Check out [our `dplyr` lesson](../dplyr/).",
         random_answer_order = TRUE,
         allow_retry = TRUE
)
```
...
```{r howmanydupids, echo=FALSE}
question("How many duplicates did you find?",
         answer("89", correct = TRUE),
         answer("86", message = "There were more than that."),
         answer("82", message = "You should have gotten a higher number."),
         answer("None", message = "There were duplicates."),
     correct = "Resplendent!",
     incorrect = "Try again.",
     random_answer_order = TRUE,
     allow_retry = TRUE
...
Let’s get started on the to-do list you just identified in the question above. We'll need the dates in `date` format. Use the `as.Date` function. Assign the dates `as.Dates` back onto themselves, then check their classes.
...
If you got "Date" for classes, you did well. 
...
```{r earliest_math, exercise = TRUE, exercise.lines = 15}
...
Now we’ll do something similar in Query 1, as far as selecting the smallest absolute value, but we’ll have to include cases where there is no date distance (it’s `NA`) to cover cases where no language date exists. We use the vertical pipe (`|`) to symbolize "or" for a statement like (exactly like) this: `is.na(date_lang - date_math)`.

See if you can put it all together in the next code block. Check the hints if you get stuck, and wrap everything in `suppressWarnings` (or `shh`) after you're happy with it.
...
You have just gone through some fairly complicated processes. Thanks go to the originator of this lesson and the accompanying data, [Joy Payton](https://github.com/pm0kjp?tab=repositories). You can find [Joy's original lesson here](https://education.arcus.chop.edu/date-pairing-in-r/).
