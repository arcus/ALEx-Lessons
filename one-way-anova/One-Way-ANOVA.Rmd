...
the [Kruskal-Wallis test](../kruskal-wallis/). Be aware of these factors if you decide to performa a Kruskal-Wallis test:
...
below 0.05 and if your data is extremely non-normal. You may even want to set alpha to something besides .05, say, .005 (see [Reassessing p Values](../reassessing-p-values/) for why this particular number might be a good choice).
...
positive may be much higher than 5%. In that case, you could use Welch’s ANOVA. If, in addition, the different groups have different _n_s, you _should_ use a [Welch's ANOVA](https://reslnarcusedu01.research.chop.edu/sb-one-way-anova/#section-welchs-anova) instead of a one-way.  
