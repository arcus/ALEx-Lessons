...
In this lesson, you'll learn how to use `vapply()` and `tapply()`, each of which serves a very specific purpose within the Split-Apply-Combine methodology mentioned in [the previous lesson](/lapply-and-sapply). For consistency, we'll use the same dataset we used in the '`lapply` and `sapply`' lesson.
