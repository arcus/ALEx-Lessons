...
Before you create a random sample, always set the seed. Use `set.seed(9699)` for each exercise in which you generate a random sample and assign it to a variable name. The random sample is not truly random but rather is generated from the seed. You can let R choose a seed, or you can set it yourself, thereby making it possible to duplicate your work later. 
...
Now repeat the command---without setting the seed---to see how your result differs. (The probability of rolling the exact same result is (1/6)^4 = 0.00077, which is pretty small!)
...
R contains a number of handy vectors, one of which is `LETTERS`. Type `LETTERS` to see it.
