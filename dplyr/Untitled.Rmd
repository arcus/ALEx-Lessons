...
To install `dplyr` use this syntax (which is typical for installing packages in R): `install.packages("dplyr")`. Remember the quotation marks, which you need for installing packages, though you don't need them for using the `library` function.

```{r a, exercise = TRUE, exercise.lines = 8}
...
To load `dplyr` into the library, type `library(dplyr)`. You don't need quotation marks this time. 
...
## Download Data for this Tutorial: Mammals Sleep
...
Let's go through some practice with `dplyr` verbs. We'll start with the two most basic functions, `select` (for choosing columns) and `filter` (for choosing rows). 
...
### Arrange or Re-Arrange Rows using `arrange`
...
Now that you've done this tutorial, I recommend you do the exercises in it again, this time without making use of the Solution button. Doing so will help anchor the ideas in your mind and make you a more fluent R user. 