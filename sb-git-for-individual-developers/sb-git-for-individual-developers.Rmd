...
"You use `git checkout` to switch between branches" (Jenny Bryan on [happygitwithr.com](http://happygitwithr.com)).

Oooooh. Okay. Let's try it.
...
One option is `git stash` (later), but generally, a better option is to safeguard the current state with a temporary `commit`. Here I use “WIP” as the commit message to indicate **w**ork **i**n **p**rogress.
...
So you just inherited this project from someone else and you were told to roll back a feature. How do you figure out when it was introduced and how you might (later) go back to it?
...
This log says that we committed once on Christmas Eve 2019 a little past 4 in the afternoon eastern time and that we said the commit "added the data file." 

Some logs are very complicated. For complex projects you probably want to install a Git client such as those listed [here](https://happygitwithr.com/git-client.html). Most of them keep the log available to you at all times as a sort of map, like this (scroll right to see all of it):
