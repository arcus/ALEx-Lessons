...
output: 
 learnr::tutorial:
    ...
    css:
      ...
      - http://reslnarcusedu01.research.chop.edu/braunsb/style.css
...
3. Use the next code chunk to save `scores` to your local directory. Replace `? # "<your filename here>"` with a full pathname pointing to where you'd like to store `scores.csv`, then execute the code. Your pathname might look something like  `"/Users/chopuser/Documents/GitHub/scores.csv"`.  

```{r save_scores, exercise = TRUE, exercise.lines = 5}
library(tidyverse)
my_filename <- ? # "<your filename here>"
write_csv(scores, my_filename)
```
```{r save_scores-solution}
library(tidyverse)
my_filename <- "/users/chopuser/documents/github/scores.csv"
write_csv(scores, my_filename)
```
...
## `git rebase`: Another way to Merge

People might tell you about putting things on top of things in Git, then show you a picture of putting things next to things. I'm a literal-minded coder, so this doesn't work for me: which means that the whole term `rebase` doesn't work for me. I can see why they chose it, but I think it brings in yet another metaphor(!) when we could have stuck with the idea of merging.  

As you know from the previous section on `git merge`, you use it to combine your efforts on two branches so you can continue along having benefitted from both.

Well, `git rebase` does the same thing, but it takes all your changes from one branch and puts them after the other branch. Use `git rebase` when you don't particularly care _when_ you made changes: you just want all the changes in one place so you can move forward.

We'll create a branch, make a change on it, then, instead of `merge`ing, we'll `rebase`. 
