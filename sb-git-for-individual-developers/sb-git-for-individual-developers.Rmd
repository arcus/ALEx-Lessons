...
See? One branch called "master" (to bring in _yet another_ metaphor), which has seen a single commit (remember doing that `git commit` just before you got to this section`? That one). 

Here is what Jenny Bryan & Co. say about branches: "Branches afford explicit workflows for integrating different lines of work on your own terms. This is much nicer than trying to do a tricky merge or rebase in a frustrated panic, because you need to push your work to GitHub at the end of the day." In other words, if people make major changes on branches, the master branch can stay as it is until everyone is sure the changes are worth keeping. The branches are still tracking the history of files, but in a different place from where the master branch has it. You can put them all together at will. 
...
question("What's a branch? Pick two.",
     ...
     correct = "Well done.",
     incorrect = "There are other commands, like `git pull` and `git diff`, that we haven't discussed yet.",
...
to create and `checkout` the branch all at once.

But wait---what do you do if you are working on a branch and need to switch, but the work on the current branch is not complete? 

One option is `git stash`, but generally, a better option is to safeguard the current state with a temporary `commit`. Here I use “WIP” as the commit message to indicate **w**ork **i**n **p**rogress.
...
At this point, we're not worried about branches. You might decide to create one, but as an individual developer, you can also just continue working with the branch you have, the "master" branch. 
