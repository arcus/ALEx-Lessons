...
Or maybe we don’t care what the increment is and we just want a sequence of 30 numbers between 5 and 10. `seq(from = 5, to = 10, length = 30)` does the trick. Give it shot now and store the result in a new variable called `my_seq`.
...
Let’s pretend we don’t know the length of `my_seq`, but we want to generate a sequence of integers from 1 to _N_, where _N_ represents the length of the `my_seq` vector. In other words, we want a new vector (1, 2, 3, …) that is the same length as `my_seq`.
