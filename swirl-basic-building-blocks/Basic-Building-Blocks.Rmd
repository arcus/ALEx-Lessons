...
In its simplest form, R can be used as an interactive calculator. Type $\displaystyle 5 + 7$ and press Cmd-Enter. On a PC, you would type Ctrl-Enter. That combination of keys executes all the code in your code chunk.
...
R simply prints the result $\displaystyle 12$ by default. However, R is a programming language and often the reason we use a programming language as opposed to a calculator is to automate some process or avoid unnecessary repetition.

In this case, we may want to use our result from above in a second calculation. Instead of retyping $\displaystyle 5 + 7$ every time we need it, we can just create a new variable that stores the result.
...
To assign the result of $\displaystyle 5 + 7$ to a new variable called `x`, you type `x <- 5 + 7`. This can be read as ‘`x` gets 5 plus 7’. Give it a try now.
...
You’ll notice that R did not print the result $\displaystyle 12$ this time. When you use the assignment operator, R assumes that you don’t want to see the result immediately, but rather that you intend to use the result for something else later on. 

If you want to *both* assign a variable *and* see the output, place parentheses around the entire assignment statment: `(x <- 5 + 7)`. 

A shortcut way to place the parentheses is to select the entire statement and then type `(`. Instead of replacing what you have blocked, a parenthesis will appear at the beginning and another will appear at the end of what you have blocked. Handy, right? R is clever like that. 

Another way to view the contents of the variable `x` is simply to type `x` and press Cmd-Enter (or Ctrl-Enter if you use a PC). Try it now.

```{r prepare-c}
x <- 5 + 7
```
```{r c, exercise=TRUE, exercise.setup = "prepare-c", exercise.lines = 5}
...
```{r d, exercise=TRUE, exercise.setup = "prepare-c", exercise.lines = 5}
...
The easiest way to create a vector is with the `c` function, which stands for ‘concatenate’ or ‘combine’. To create a vector containing the numbers $\displaystyle 1.1, 9,$ and $\displaystyle 3.14$, type `c(1.1, 9, 3.14)`. Try it now and store the result in a variable called `z`.
...
When you have questions about a particular function, you can access R’s built-in help files via the `?` command. For example, if you want more information on the `c` function, type `?c` without the parentheses that normally follow a function name. Give it a try.
...
You can combine vectors to make a new vector. Create a new vector that contains `z`, $\displaystyle 555$, then `z` again in that order. Don’t assign this vector to a new variable, so that we can just see the result immediately.
...
Other common arithmetic operators are `+`, `-`, `/`, and `^` (where `x^2` means ‘ex squared’). To take the square root, use the `sqrt` function and to take the absolute value, use the `abs` function.
