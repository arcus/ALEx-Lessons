...
In its simplest form, R can be used as an interactive calculator. Type `5 + 7` and press `Cmd-Enter`. On a PC, you would type Ctrl-Enter. That combination of keys executes all the code in your code chunk.
...
R simply prints the result `12` by default. However, R is a programming language and often the reason we use a programming language as opposed to a calculator is to automate some process or avoid unnecessary repetition.
...
You’ll notice that R did not print the result `12`this time. When you use the assignment operator, R assumes that you don’t want to see the result immediately, but rather that you intend to use the result for something else later on. 
...
What is the value of `y`? Type `y` and run the code to find out.
...
question("Which statement do you think is true?",
         ...
         correct = "Awesome!",
         incorrect = "Read some of what came before this question to find the answer. It's there.",
         random_answer_order = TRUE,
         allow_retry = TRUE
)
```
