...
Put `Hmisc` in the library. We'll use one of R's prepackaged data sets as input, `mtcars`. We have to pass `mtcars` to `rcorr` as a matrix, not as a data frame. Nest a call to `as.matrix` with input `mtcars` within a call to `rcorr`. You don't have to pass `rcorr` anything else. 
...
Now remove the line that nullifies the upper panel and put in its stead `upper.panel = panel.pie`. 
...
Let's perform another linear regression analysis and add an additional step. 
As before, call `lm` and pass it the formula `Eggs ~ Weight` (which is $\displaystyle Y$ ~ $\displaystyle X$) and the data, `egg`. Save it to the variable `egg_fit`, then pass `egg_fit` to `summary`. 
