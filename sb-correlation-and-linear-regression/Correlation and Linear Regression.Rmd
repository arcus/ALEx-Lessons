...
In R, everything is an object. Use `str` (short for "structure"---and so many other things, but here all we care about is "structure") to find out what the linear regression best-fit output `egg_fit` looks like to R.
...
So after we run an `lm`, R gives us an object that contains the details of the analysis. We will look at the details in a minute, but first note that you can use output very nicely in your [reproducible Markdown text](https://education.arcus.chop.edu/scripted-analysis/) by calling it within text while you write a Markdown document. 

For instance, you might write, "My statistic is " or "My residuals are " and then type a tic-mark ("`", likely your lowercase "~", in the upper left corner of your keyboard, not to be confused with a single quote), then the text "r coregg\$statistic" (or "r egg_fit\$residuals") without quotation marks, and finally, a closing tic-mark. The rendered (knitted) output would be 

>My statistic is `r coregg$statistic`. My residuals are `r egg_fit$residuals`. 

Now let's look at elements of R's output. This may be more detail than you want, so feel free to skip it. But if you have ever wondered what linear model output actually *means*, take a little time to work through the rest of these exercises. You can also use this lesson as a reference tool by looking up the elements as you find you need to know what they are.

To examine the objects in a best fit model or correlation, start by making a list of all the put outted (outputted?) objects. Call `names`, and pass `summary(egg_fit)`.

Also, either before or after you do that, give yourself a simple way to type `summary(egg_fit)` by assigning it to `sef`.
...
We've made ourselves a to-do list! Let's go through the objects one by one.

To review how we got here:

1. You put the `egg` linear regression output in `egg_fit`    
2. You looked at the summary of that output by calling `summary(egg_fit)`    
3. You stored `summary(egg_fit)` in a variable, `sef`. 

Now you can look at each variable within `sef` to find the details of the linear model you created with your original call to `lm`. In fact, the first object we'll look at is where R stored the information about that call.  

### `call`

Simply type sef$call to see the original formula you passed to `lm`. 
...
Note the $\displaystyle X$ axis. See how it goes from -5 to 10?  The skewness is to the right. The data looks sort of normal, and it might be if the axis was from -5 to 5.

There is a simple solution to this problem, and I really should put it in a lesson all by itself, but we're here now so here it is: To fix skewness, create a new variable for your independent variable which is the natural log of the original data.
...
What this means: `ln_Weight` is not collinear with `egg$Eggs`. 
...
```{r sigma-lm2, exercise = TRUE, exercise.lines = 8}

```
```{r sigma-lm2-solution}
round(sd(slef$coefficients[1:2]), 1)
```

It's close enough to `sigma` to say the differences are due to rounding in the guts of the calculations. 
...
```{r r-squared-lm, exercise = TRUE, exercise.lines = 8}

```
```{r r-squared-lm-solution}
slef$r.squared
```
...
Linear regression and ANOVAs use an _F_ statistic to calculate _p_ values. You should report this statistic with any reference to your model, along with the degrees of freedom stored with it. 

```{r fstat-lm, exercise = TRUE, exercise.lines = 5}
...
A correlation is a standardized or scaled measure of the strength of the relationship between two variables. Covariance is the same thing, but in the original units of measure for the variables. It is reported in a table format. 

```{r cov-unscaled-lm, exercise = TRUE, exercise.lines = 5}

```
```{r cov-unscaled-lm-solution}
slef$cov.unscaled
```
...
As previously noted, plot easily using plain old `plot` and giving it your data set (if you have only a few variables) to see what it makes of it. Try this now with `diversity`. 
...
That is a great way to start looking at a data set. You can look at each plot and decide whether any of the images make you think there might be correlations between the two variables involved. For instance, on the bottom row, second from the right, is an interesting image that may or may not show a correlation. This initial plot helps you decide how to proceed. 
