...
The point is to remember that you can manipulate $\displaystyle r^2$ by choosing the range of your input variables. We'll talk about this more when we address multivariate analysis. 
...
Of course you can do many and varied things with plots. Rather than trying to do something better than has already been done excellently, I recommend you start with [this wonderful `ggplot2` tutorial](http://r-statistics.co/ggplot2-Tutorial-With-R.html). For a simpler approach, check out the [interactive base plotting system lesson](), which is also based on the work of others, the wonderful people at [swirl.org]().

## I _like_ Math. Where's the Math? How Correlations and Linear Regressions Work
...
where $\displaystyle a$ is the $\displaystyle Y$ intercept and b is the slope. Once you know a and b, you can use this equation to predict the value of $\displaystyle Y$ for a given value of $\displaystyle X$. For example, the equation for the heart rate-speed experiment is `rate` = 63.357 + 3.749 × `speed`. I could use this to predict that for a `speed` of 10 kph, my heart `rate` would be 100.8 bpm. 

You should do this kind of prediction within the range of $\displaystyle X$ values found in the original data set (interpolation). Predicting $\displaystyle Y$ values outside the range of observed values (extrapolation) is sometimes interesting, but it can easily yield ridiculous results if you go far outside the observed range of $\displaystyle X$. In the frog example below, you could mathematically predict that the inter-call interval would be about 16 seconds at −40°C. Actually, the inter-calling interval would be infinity at that temperature, because all the frogs would be frozen solid.
